    ...
    : Anwill::Layer(ups), m_IsRound(false), m_Player(Anwill::Ecs::CreateEntity()), m_Camera(ws.width, ws.height)
{
    ...
    Anwill::Ecs::AddComponent<Anwill::RBody>(npc3, 5.0f, Anwill::Math::Vec3f(200.0f, 600.0f, 0.0f));
    Anwill::Ecs::AddComponent<Anwill::Math::Mat4f>(npc3);
    Anwill::Ecs::AddComponent<Anwill::RBody>(npc4, 5.0f, Anwill::Math::Vec3f(200.0f, 300.0f, 0.0f));
...
void CollisionPoints::Update(const Anwill::Timestamp& timestamp)
{
    ...
    Anwill::Ecs::ForEach<Anwill::Math::Mat4f, Anwill::RBody>([this](Anwill::EntityID id,
        ...
        if(id == m_Player and m_IsRound)
        {
            Anwill::Renderer::Submit(m_CircleShader, m_Mesh, transform);
        } else if(id == m_Player and !m_IsRound) {
        ...
        if (id == m_Player)
        {
            MoveAndTiltPlayer(body, transform);
        }
    });
...
void CollisionPoints::MoveAndTiltPlayer(Anwill::RBody& pBody, Anwill::Math::Mat4f& transform)
{
    float speed = 0.1f;
...
void CollisionPoints::SwapShape()
{
    ...
    {
        Anwill::Ecs::ForEntity<Anwill::RBody>(m_Player, [](Anwill::RBody& body) {
            body.SetCollider<Anwill::CircleCollider>(40.0f);
        });
        m_IsRound = true;
    }
    ...
    {
        Anwill::Ecs::ForEntity<Anwill::RBody>(m_Player, [this](Anwill::RBody& body) {
            auto vs = m_Mesh.GetVertices();
            body.SetCollider<Anwill::PolygonCollider>(vs);
        });
        m_IsRound = false;
