...
namespace Anwill {
    ...
    public:
        ...
        GuiSlider(const std::shared_ptr<GuiStyling::Container>& containerStyle, T min, T max, T& source,
            ...
            : GuiElement(containerStyle), m_Min(min), m_Max(max), m_Reference(source),
              ...
              m_Style(AW_GUI_CAST_STYLE(GuiText::m_Style, GuiStyling::Slider))
        {
            if(std::is_same<T, float>::value)
            {
                ...
                m_SetValueText = [this](){ GuiText::SetText(Math::RoundToString(m_Reference, 2)); };
            }
            else if(std::is_same<T, int>::value)
            {
                m_OnPressSet = [](float f){ return std::round(f); };
                m_SetValueText = [this](){ GuiText::SetText(std::to_string(m_Reference)); };
            } else {
                    ...
                    const Timestamp& delta) override
        {
            ...
            float leftMostSliderPos = m_MarkerXOffset + 1.0f;
            float rightMostSliderPos = GuiButton::GetWidth() - m_MarkerXOffset - 1.0f;
            // Compute slider X position based on reference value
            float markerXPosDelta = Math::ScaleToRange<float>(static_cast<float>(m_Reference),
                                                              leftMostSliderPos,
                                                              rightMostSliderPos,
                                                              static_cast<float>(m_Min),
                                                              static_cast<float>(m_Max));
            markerXPosDelta = Math::Clamp(markerXPosDelta, leftMostSliderPos, rightMostSliderPos);
            // Define 2D position. The button
            Math::Vec2f markerPos = {markerXPosDelta - m_MarkerXOffset,
                                     -(GuiButton::GetHeight() -
            GuiStyling::Slider::GetMarkerSize(m_HostContainerStyle->elementHeight).Y) * 0.5f};
